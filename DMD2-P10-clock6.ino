...
ver.5a - temperature with big characters
ver.6 - added 12-hour format or 24-hour format
--------------------------------------------------------------------------------------*/
...
byte tip = 1;   // 0 = thin (Arial14 font)
...
int tempC1;
int tz, tu, ts;
byte h12 = 0; // 0 -> 24-hour format, 1 -> 12-hour format
byte pm = 1;  // 0 -> am, 1 -> pm 
...
//dmd.drawChar(x,y, 58);  // letter C - for test with changed font
}
}
if (ora2 != ora20)
{
...
dmd.drawChar(x+18,y+8, '%');  
  ...
  else
  {
    ...
    dmd.selectFont(Font_6x14);
        if (tz != 0)  
           dmd.drawChar(x,y, 48+tz);
        else
           dmd.drawChar(x,y, ' ');
        dmd.drawChar(x+7,y, 48+tu);
        dmd.drawLine(14,13,14,14);
        dmd.drawLine(15,13,15,14);
        dmd.drawChar(x+17,y, 48+ts);
        dmd.drawLine(24,3,24,4);
        dmd.drawLine(25,3,25,4);
        dmd.drawChar(x+27,y, 58);  // bold C  
  }
}
//dmd.clearScreen();   //true is normal (all pixels off), false is negative (all pixels on)
}
...
if (stare == 12)
{
  dmd.drawChar(x,y, 'f');
  dmd.drawChar(x+5,y, 'o');
  dmd.drawChar(x+10,y, 'r');
  dmd.drawChar(x+15,y, 'm');
  dmd.drawChar(x+20,y, 'a');  
  dmd.drawChar(x+25,y, 't');  
  dmd.drawChar(x+30,y, ':');  
...
if (digitalRead(meniu) == LOW)
 ...
 //h12 = h12%2;
  if (h12%2 != 0) 
  {
    dmd.drawChar(x+10,  y+8, '1');
    dmd.drawChar(x+15,y+8, '2');
  } 
  else
    {
    dmd.drawChar(x+10,  y+8, '2');
    dmd.drawChar(x+15,y+8, '4');
  } 
    dmd.drawChar(x+20,y+8, 'h');
} // end change hour
